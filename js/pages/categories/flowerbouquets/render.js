/* render module for flowerbouquets */
...
function updateProductsCount(count) {
            ...
            if (countElement) {
                const productCount = count || filteredProducts.length;
                countElement.textContent = `${productCount} منتج`;
            }
        }
...
function showNotification(message, type = 'info') {
        ...
        let filtersOpen = false; // متغير لتتبع حالة الفلاتر
...
function displayCategoryFeaturedAds(ads) {
            ...
            const adsHTML = ads.map(ad => {
                const imgs = Array.isArray(ad.image_urls) && ad.image_urls.length>0 ? ad.image_urls : [ad.image_url || '../assets/images/f.jpg'];
                return `
                <div class="bg-white rounded-lg shadow-md overflow-hidden border-2 border-yellow-200 hover:border-yellow-400 transition-all duration-300 transform hover:scale-105 flex-shrink-0 cursor-pointer" style="width: 200px;" data-ad-id="${ad.id}" data-product-id="${ad.id}" data-ad-info='${JSON.stringify(ad)}' onclick="viewProductDetails('${ad.id}')">
                    <div class="relative image-container" data-product-id="${ad.id}" tabindex="0">
                        ...
                        <button onclick="toggleFavorite(event, '${ad.id}', '${ad.title}', '${ad.image_url || '../assets/images/f.jpg'}', ${ad.price || 0}, '${ad.governorate || ''}', '${ad.cities || ''}', '${ad.subcategories ? ad.subcategories.join(',') : ''}')" class="absolute top-1 left-1 bg-white rounded-full p-1.5 shadow-lg hover:shadow-xl transition-all duration-200 favorite-btn border-2 border-white"><svg class="w-4 h-4 ${isFavorite('${ad.id}') ? 'text-pink-500 fill-current' : 'text-gray-400'}" viewBox="0 0 24 24" fill="currentColor"><path d="M12 21.35l-1.45-1.32C5.4 15.36 2 12.28 2 8.5 2 5.42 4.42 3 7.5 3c1.74 0 3.41.81 4.5 2.09C13.09 3.81 14.76 3 16.5 3 19.58 3 22 5.42 22 8.5c0 3.78-3.4 6.86-8.55 11.54L12 21.35z"/></svg></button>
                    </div>
...
function openWhatsAppChat(productId, title, imageUrl, price, governorate, cities, subcategories, whatsappNumber) {
            ...
            if (imageUrl && imageUrl !== '../assets/images/f.jpg') {
        ...
        document.addEventListener('DOMContentLoaded', function() {
            ...
            // تأكيد تهيئة كائن الفلاتر قبل استخدامها
            ...
            console.log('URL params:', { subcatParams, govParams, cityParams, colorParams, pmin, pmax });

            // تطبيق الفلاتر من URL
            if (subcatParams.length > 0) {
                window.activeFilters.subcategory = subcatParams;
            }
            if (govParams.length > 0) {
                window.activeFilters.governorates = govParams;
            }
            if (cityParams.length > 0) {
                window.activeFilters.cities = cityParams;
            }
            if (colorParams.length > 0) {
                window.activeFilters.colors = colorParams;
            }
            if (pmin !== null || pmax !== null) {
                window.activeFilters.price = { min: pmin, max: pmax };
            }

            console.log('Applied filters to window.activeFilters:', window.activeFilters);
            ...
            // Active filters object is already initialized above with URL parameters

            // Apply filters from URL after products are loaded (will be called in loadProducts)
            // Filters will be applied after products load in loadProducts() function
            ...
            // انتظار تحميل ProductService قبل تحميل المنتجات
            const waitForProductService = () => {
                if (window.ProductService && window.ProductService.initialized) {
                    // تحميل المنتجات
                    loadProducts();
                    ...
                    // حدث زر تحميل المزيد - تحقق من وجود العنصر أولاً
                    const loadMoreBtn = document.getElementById('load-more-btn');
                    if (loadMoreBtn) {
                        loadMoreBtn.addEventListener('click', loadMoreProducts);
                    }
                } else {
                    // انتظار 100ms ثم المحاولة مرة أخرى
                    setTimeout(waitForProductService, 100);
                }
            };
