...
function displayProducts(products) {
            ...
            console.log('Displaying products:', products ? products.length : 0, 'products');

            // Hide loading state
            loadingState.classList.add('hidden');

            if (!products || products.length === 0) {
                const message = activeFilters.price || activeFilters.subcategory.length > 0 || activeFilters.governorate.length > 0 || activeFilters.cities.length > 0
                    ? 'لا توجد منتجات تطابق معايير البحث المحددة'
                    : 'لا توجد منتجات متاحة حالياً';
                container.innerHTML = `<div class="text-center py-12"><p class="text-gray-500 text-lg">${message}</p></div>`;
