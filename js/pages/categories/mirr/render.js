...
function displayProducts(products) {
            ...
            container.innerHTML = currentProducts.map(product => {
                ...
                // Get subcategory display name using the centralized translation system
                let subcategoryNames = [];
                let subcategoryColors = [];

                // Function to get color based on subcategory name
                function getSubcategoryColor(subcategoryName) {
                    const colorMap = {
                        'مرايا زفاف': 'bg-pink-500',
                        'مرايا خطوبة': 'bg-purple-500',
                        'مرايا ديكور': 'bg-green-500',
                        'مرآة زفاف': 'bg-pink-500',
                        'مرآة خطوبة': 'bg-purple-500',
                        'مرآة ديكور': 'bg-green-500',
                        'مرآة مخصصة': 'bg-indigo-500',
                        'مرآة كلاسيكية': 'bg-yellow-500',
                        'مرآة عصرية': 'bg-blue-500',
                        'مرآة فاخرة': 'bg-red-500'
                    };
                    return colorMap[subcategoryName] || 'bg-gray-500';
                }
                ...
                if (product.subcategory) {
                    ...
                    if (Array.isArray(product.subcategory)) {
                        ...
                        try {
                            let cleanString = product.subcategory.trim();
                            ...
                            subcategories = Array.isArray(parsed) ? parsed : [cleanString];
                        } catch (e) {
                            // تنظيف النص من علامات الاقتباس الإضافية
                            let cleanText = product.subcategory.trim();
                            ...
                            subcategories = cleanText.split(',').map(s => s.trim()).filter(Boolean);
                        }
                    }

                    subcategories.forEach(sub => {
                        const translatedName = translateSubcategory(sub);
                        if (translatedName && !subcategoryNames.includes(translatedName)) {
                            subcategoryNames.push(translatedName);
                            subcategoryColors.push(getSubcategoryColor(translatedName));
                        }
                    });
                }
                    ...
                    <div class="p-6 overflow-y-auto max-h-[calc(90vh-80px)]">
                        ...
                        <div class="flex gap-3 mt-6 pt-6 border-t">
                            ...
                            <button onclick="toggleFavoriteFromModal('${product.id}')"
                                    class="w-full bg-pink-500 hover:bg-pink-600 text-white py-3 px-4 rounded-lg font-medium transition-colors text-center">
...
function displayCategoryFeaturedAds(ads) {
            ...
            const adsHTML = ads.map(ad => {
                const imgs = Array.isArray(ad.image_urls) && ad.image_urls.length>0 ? ad.image_urls : [ad.image_url || '../assets/images/sarah-penney-1yk6cjyije8-unsplash.jpg'];
                return `
                <div class="bg-white rounded-lg shadow-md overflow-hidden border-2 border-yellow-200 hover:border-yellow-400 transition-all duration-300 transform hover:scale-105 flex-shrink-0 cursor-pointer" style="width: 200px;" data-ad-id="${ad.id}" data-product-id="${ad.id}" data-ad-info='${JSON.stringify(ad)}' onclick="viewProductDetails('${ad.id}')">
                    <div class="relative image-container" data-product-id="${ad.id}" tabindex="0">
                        ...
                        ${imgs.length>1?`
                            <button class="slider-btn slider-prev" aria-label="السابق" onclick="event.stopPropagation(); event.preventDefault(); handleSliderClick(this, 'prev');">‹</button>
                            <button class="slider-btn slider-next" aria-label="التالي" onclick="event.stopPropagation(); event.preventDefault(); handleSliderClick(this, 'next');">›</button>
                        `:''}
                        ...
                        <div class="absolute top-1 left-1"><button class="favorite-btn bg-white rounded-full p-1.5 shadow-lg hover:bg-pink-50 transition-all duration-200" onclick="toggleFavorite(event, '${ad.id}', '${ad.title || 'منتج مميز'}', '${ad.image_url || '../assets/images/sarah-penney-1yk6cjyije8-unsplash.jpg'}', '${ad.price || 0}', '${ad.governorate || ''}', '${ad.cities || ''}', '${ad.subcategories ? ad.subcategories.join(',') : ''}')"><svg class="w-4 h-4 ${isFavorite(ad.id) ? 'text-pink-500 fill-current' : 'text-gray-400'}" viewBox="0 0 24 24"><path d="M12 21.35l-1.45-1.32C5.4 15.36 2 12.28 2 8.5 2 5.42 4.42 3 7.5 3c1.74 0 3.41.81 4.5 2.09C13.09 3.81 14.76 3 16.5 3 19.58 3 22 5.42 22 8.5c0 3.78-3.4 6.86-8.55 11.54L12 21.35z"/></svg></button></div>
                        </div>
                    <div class="p-3"><div class="flex items-center justify-between mb-2"><span class="text-sm font-bold text-yellow-600">${ad.price > 0 ? ad.price + ' ج.م' : 'السعر عند الطلب'}</span><span class="text-xs text-gray-500">${ad.governorate}</span></div><div class="flex gap-1"></div></div>
                </div>`;
            }).join('');
